...
const EventItem: React.FC<EventItemType> = ({ item, onPress }) => {
  ...
  const getImage = (!item.english_image) ? AddHTTPS(item.image)
    : AddHTTPS(item.english_image);
  const eventName = ((!!item.english_name) ? `${item.english_name}\n` : '')
    + item.japanese_name;
  ...
  return <Surface style={[styles.container, { backgroundColor: state.color }]}>
    <TouchableRipple onPress={onPress}
      background={TouchableNativeFeedback.Ripple(state.color, false)}>
      <>
        <FastImage onLoad={onLoad}
          source={{
            uri: getImage,
        ...
        <View style={styles.textBox}>
          <Text style={styles.text}>
            {(state.label.length > 0 ? (`[${state.label.toUpperCase()}]\n`) : '') + eventName}
          </Text>
        </View>
      </>
    </TouchableRipple>
